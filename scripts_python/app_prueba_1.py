...
def hacer_entrega(id_usuario,id_empleado,id_epp,cantidad,bodega,razon,cur,conn):                  # función para hacer una retiro de ep
    try:
        ...
        sql_insertar = '''
                            INSERT INTO entrega (id_usuario, id_empleado, id_epp, precio_en_transaccion, cantidad, fecha_y_hora, lugar)
                            VALUES (%s, %s, %s, %s, %s, %s, %s)
                        '''
        if razon != '':
            sql_insertar = '''
                                INSERT INTO entrega (id_usuario, id_empleado, id_epp, precio_en_transaccion, cantidad, fecha_y_hora, lugar,razon)
                                VALUES (%s, %s, %s, %s, %s, %s, %s,%s)
                            '''     
            cur.execute(sql_insertar, [id_usuario, id_empleado, id_epp, precio, cantidad, fecha_hora, bodega,razon])
        else:
            cur.execute(sql_insertar, [id_usuario, id_empleado, id_epp, precio, cantidad, fecha_hora, bodega])
...
def main(usuario_id):
    ...
    listbox_entregas_1 = Listbox(buscar_entregas, width=180, height=10) # se crea una listbox que muestra los epps
    listbox_entregas_1.grid(row=0, columnspan=14, sticky= W+E)      # se posiciona la listbox

    for entrega in entregas:             # se rellena la listbox con los epps
        listbox_entregas_1.insert(END, entrega)     
