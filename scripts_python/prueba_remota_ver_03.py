...
def main(usuario_id,permisos_id):
    ...
    def modificar_stock_3():
        ...
        try:    # en un try por si falla
            if(int(cantidad)>=0):   # si la cantidad es mayor a 0
                ...
                res = modificar_stock_epp(id,cantidad,bodega,cur,conn,True)   # se ejecuta la consulta
            else:
                res = -2    # error al poner un número menor a 0 
        except:     # si falla
            res = -1    # código de error
        ...
        if(res == 1):
            ...
            hacer_entrega(usuario_id, usuario_id, id, int(cantidad)-cantidad_antigua, bodega, 'Stock modificado manualmente', cur,conn, True) 
    ...
    entrega_rapida = Frame(tabs_entrega)    # se crea y se le da como padre las tabs de entregas
    tabs_entrega.add(entrega_rapida, text='Entrega rápida')

    # id empleado

    label_empleado_id_entrega_3 = Label(entrega_rapida, text = 'ID empleado:')  # id del empleado
    label_empleado_id_entrega_3.grid(row=0,column=0)

    entry_empleado_id_entrega_3 = Entry(entrega_rapida) # entry para el id
    entry_empleado_id_entrega_3.grid(row=0,column=1)

    # bodega

    label_bodega_entrega_3 = Label(entrega_rapida, text='Bodega:')
    ...
    boton_cambiar_bodega = Button(entrega_rapida, text='Cambiar bodega', command=cambiar_bodega_entrega_3)
    boton_cambiar_bodega.grid(row=0,column=4)

    # cantidad

    label_cantidad_entrega_3 = Label(entrega_rapida, text='Cantidad:')
    label_cantidad_entrega_3.grid(row=0,column=5)

    entry_cantidad_entrega_3 = Entry(entrega_rapida)    # entry paara la cantidad
    ...
    label_leyenda_entrega_3_1 = Label(entrega_rapida,text='PARÁMETROS:')
    label_leyenda_entrega_3_1.grid(row=1,column=0)

    label_leyenda_entrega_3_2 = Label(entrega_rapida,text='ID-BODEGA-NOMBRE-TALLA-STOCK DISPONIBLE')
    ...
    boton_entregas_3 = Button(entrega_rapida,text='Realizar transacción',command=seleccionados_entrega_3)   # botón para realizar transacción
    boton_entregas_3.grid(row=3,column=0)

    label_status_transaccion_entrega_3 = Label(entrega_rapida, text='')
